...
class All {
   public:
    ...
    uint64_t rankSTL(uint64_t val);  // STL std::lower_bound baseline
...
uint64_t All::predSearch(uint64_t val) {
    ...
    // for(int i = 0; i < size; i++) if(extractedinput[i] >= val) return i;
    // return size - 1;
}
...
uint64_t All::rankSTL(uint64_t val) {
    // STL std::lower_bound baseline - what most programmers would use
    auto it = std::lower_bound(input, input + size, val);
    if (it == input + size) return size - 1;
    return it - input;
}
...
int main(int argc, char *argv[]) {
    ...
    if ((size * nbb) <= 256) {
        ...
        auto sumAFK = 0, sumFW = 0, sumFWM = 0,sumFWMI = 0, sumLinear = 0, sumBSI = 0, sumSTL = 0;
        auto timeAFK = 0, timeFW = 0, timeFWM = 0,timeFWMI = 0, timeLinear = 0, timeBSI = 0, timeSTL = 0;
        ...
        // STL std::lower_bound baseline
        start_time = std::chrono::high_resolution_clock::now();
        for (int i = 0; i < n; i++) {
            sumSTL += pat_tree.rankSTL(A[i]);
        }
        timeSTL = std::chrono::duration_cast<std::chrono::microseconds>(
                         std::chrono::high_resolution_clock::now() - start_time)
                         .count();
        ...
        cout << size << "\t" << nbb << "\t" << timeLinear  << "\t" << timeSTL << "\t" << timeAFK
             << "\t" << timeFWM << "\t" << timeFWMI << "\t" << timeBSI<< "\t["
             << (sumLinear+sumSTL+sumAFK-sumLinear+sumFWM-sumLinear+sumFWMI/
                  sumLinear/sumBSI)
             << "]" << endl;
