...
/**
 * The <tt>EliasFanoAppendOnlyMonotoneLongSequence</tt> class represents a
 * monotone sequence of non-decreasing integers compressed with the
 * <em>Elias-Fano integer encoding</em>. The implementation maintains a
 * collection of buckets of size B integers that are all statically compressed
 * with the Elias-Fano strategy. It is responsibility of the user to provide the
 * proper value of B to maximize compression. This is possible if we know how
 * many elements the sequence is expected to contain.
 *
 * <p>
 * It supports the <em>append</em>, <em>get</em> and <em>next greater or
 * equal</em> operations, along with methods for inspecting how many bits the
 * sequence is using, testing if the sequence is empty, and iterating through
 * the items in order.
 * </p>
 *
...
//    return -1L;
    ...
    public int bits() {
        ...
        for (int i = 0; i < buckets; i++) {
            bits += lowerBits.get(i).length * LONG_SIZE + selectors.get(i).numBits() + selectors.get(i).bitVector().length();
        }
    ...
    public long randomAccess(int[] pos) {
        long sum = 0;
