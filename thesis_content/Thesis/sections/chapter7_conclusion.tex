...
\begin{itemize}
    \item An analysis demonstrating that using a growth factor of $f = n/w$ (where $n$ is the current size and $w$ is the word size) achieves amortised $O(1)$ append time while limiting internal fragmentation to $o(n)$ space.

    \item An append-only dynamic bitvector implementation for the special case of $width=1$, supporting rank and select operations with $n + o(n)$ space usage.
...
\begin{itemize}
    ...
    \item A space-efficient design using $O(n \log n) + o(n)$ bits while maintaining competitive query performance.
\end{itemize}
...
\textbf{Hardware Evolution}: The feasibility of implementing constant-time predecessor search was enabled by advances in processor instruction sets (AVX2, BMI2). Algorithms that were impractical two decades ago are now implementable and efficient. This suggests that theoretical algorithms should be periodically re-evaluated as hardware capabilities evolve.
