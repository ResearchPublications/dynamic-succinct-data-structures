...
\end{lemma}\par
\textit{Proof:}
Two adjacent integers $(x, y) \in S$ shares the longest common bits compared to any other combination of $(x, z) \in S$, which makes the lemma to hold trivially. If $(x, z)$ has an $lcp$ greater than $(x, y)$, then $z < y$, and that violates the sorting order of $S$.\par
\textbf{Example 1.}
\begin{itemize}
...
\begin{itemize}
    \item \textit{Load and store}: this group of instructions load and copy data from continuous chunk of memory into a SIMD register. When the data to be read is from a non-continuous memory locations, this slows down these operations as you will need a separate $read$ operation for each location.
    ...
    \item \textit{Horizontal-wise}: these sets of instructions act across the elements of a SIMD register. It allows for operations to be performed on values store within a register. For example, adding or multiplying two values within the same register and permutation of values within a register.
    \item \textit{Advanced BMIs}: these are bit manipulation instructions implemented as single instructions in $intel$ processors that were considered expensive before. These includes $PDEP$, and $PEXT$.
\end{itemize}
...
\begin{table}[ht]
    ...
    \end{minipage}%
\end{table}
The predecessor of $x$ can be trivially computed by returning $S[rank(x, S)]$, and the successor of $x$ as $S[rank(x, S) + 1]$. All operations are supported in constant time. The major drawback to this apparoach is the extra space incurred of $O(2^b)$, which is largely dependent on the number of significant bit positions.
\par
\subsection{Fredman's Method} 
