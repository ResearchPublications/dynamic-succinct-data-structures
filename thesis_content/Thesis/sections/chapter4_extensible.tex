...
The novel contributions of this chapter are: (i) an analysis of resize factors showing that $f = n/w$ achieves amortised $O(1)$ resize time while limiting internal fragmentation to $o(n)$ bits; (ii) a practical append-only bitvector supporting rank and select with $n + o(n)$ bits of space; (iii) an extensible Elias-Fano implementation that operates when the universe $u$ is unknown beforehand, maintaining space close to the Information Theoretic Lower Bound; and (iv) empirical benchmarks validating these results against standard library implementations.
\section{Extensible Array}
 Arrays are excellent examples of static data structures that have been studied extensively \cite{Navarro_2016}. Vector is the most basic form of extendible array that supports random access in constant time. They have been shown to solve a number of dynamic data structure problems such as randomised queues, stacks, priority queues and queues \cite{Brodnik_1999, Navarro_2016, Brodnik_1993}, and have been used in building complex compact data structures \cite{Munro_2001, Raman_2003}. An Extendible Array (EA) built upon the idea of vectors is described in this chapter. Other variants of EAs of non-standard widths (such as bit-vectors, shorts, characters, etc.) are also presented. The operations supported are:
\begin{itemize}
...
\begin{itemize}
    ...
    \item An analysis of resize factors demonstrating that by choosing $f = n/w$ (where $n$ is the current number of bits and $w$ is the word size), amortised $O(1)$ resize time can be achieved while limiting internal fragmentation to $o(n)$ bits, as shown in Equations~\ref{eq:1} and~\ref{eq:2}.

    \item An append-only bitvector implementation supporting rank and select operations with $n + o(n)$ bits, building upon the blocking techniques of Jacobson~\cite{Jacobson_1989} and Clark~\cite{Clark_1998}.
